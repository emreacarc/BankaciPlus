"""
Credit Risk Lite Model Eğitim Scripti
XGBoost Classifier - 8 değişken (7 temel + 1 türetilmiş)
Hızlı ön tarama için optimize edilmiş model
"""
...
print("=" * 80)
print("CREDIT RISK LITE MODEL EĞİTİMİ")
print("=" * 80)
print()
...
# Türetilmiş özellik: loan_to_income
df['loan_to_income'] = df['loan_amnt'] / (df['annual_inc'] + 1)

# Lite Model için 8 değişken seçimi (7 temel + 1 türetilmiş)
lite_features = [
...
categorical_features = ['home_ownership', 'purpose', 'grade']

# Hedef değişken
target = 'loan_status_binary'  # 0: Ödendi, 1: Temerrüt

# Veri hazırlama
X = df[lite_features + categorical_features].copy()
y = df[target].copy()

print(f"   Seçilen özellik sayısı: {len(lite_features)} temel + {len(categorical_features)} kategorik")
...
feature_names = lite_features + list(preprocessor.named_transformers_['cat'].get_feature_names_out(categorical_features))
...
print("   RandomizedSearchCV ile 150 kombinasyon test ediliyor (3-fold CV)...")
...
output_file = 'credit_risk_lite_model.pkl'
